...
interface ProfessorCourseViewProps {
...
const ProfessorCourseView: React.FC<ProfessorCourseViewProps> = ({ user }) => {
  ...
  useEffect(() => {
      const fetchGroupDetails = async () => {
          if (idGrupo === 0) {
              setErrorGroup("ID de grupo no válido.");
              setLoadingGroup(false);
              return;
          }
          try {
              
              const response = await axios.get<GroupDetail>(`http://localhost:${port}/api/Grupo/${idGrupo}`);
   ...
   if (loadingGroup) {
        return <div className="text-center mt-8 text-gray-600">Cargando información del grupo...</div>;
    }
            ...
            {activeTab === 'noticias' && (
             ...
             {activeTab === 'documentos' && (
                <div className="p-4 rounded-lg bg-gray-50" role="tabpanel" aria-labelledby="documentos-tab">
                    ...
                    <ProfessorDocumentExplorer idGrupo={idGrupo} user={user} />
                </div>
            )}
